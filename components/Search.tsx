...
import {defaultSearchEngines} from "~scripts/popup/settingsConfig";
import {
  SEARCH_ENGINE,
...
export function Search() {
  const [searchEngine, _] = useStorage(SEARCH_ENGINE, JSON.parse(defaultSearchEngines[0].value));
  const [searchEngineTextColor] = useStorage(SEARCH_ENGINE_TEXT_COLOR, SEARCH_ENGINE_TEXT_COLOR__DEFAULT);
  const [searchEnginePlaceholderColor] = useStorage(SEARCH_ENGINE_PLACEHOLDER_COLOR, SEARCH_ENGINE_PLACEHOLDER_COLOR__DEFAULT);
  const [searchEngineBackgroundColor] = useStorage(SEARCH_ENGINE_BACKGROUND_COLOR, SEARCH_ENGINE_BACKGROUND_COLOR__DEFAULT);
  ...
  return (
    <div className="relative">
      <input
        ...
        onKeyDown={(e) => {
          if (e.key === "Enter" ) {
            window.location.replace(`${searchEngine.link.replaceAll("%input%", e.currentTarget.value)}`);
          }
        }}
      />
      <img
