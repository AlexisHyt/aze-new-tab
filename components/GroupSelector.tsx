      ...
      <div ref={containerRef}>
        ...
        {groups.length > 0 && (
          <>
            ...
            >
              {activeGroup.logo && (
                <img src={activeGroup.logo} alt={activeGroup.name} className={`h-4`} />
              )}
              {activeGroup.name}
            </button>
            {isDropdownOpen && (
              <div className="absolute rounded-md bg-black mt-2 w-auto flex flex-col gap-2 z-50">
                {groups.map((group, index) => (
                  ...
                  >
                    {group.logo && (
                      <img src={group.logo} alt={group.name} className={`h-3`} />
                    )}
                    {group.name}
                    {group.name === activeGroup.name ? (
                      <span className={`bg-blue-500 rounded-md px-1`}>active</span>
                    ) : null}
                  </button>
                ))}
              </div>
