...
import type { IRSSFeed } from "~scripts/popup/types";
import {
  ACTIVE_RSS_FEED,
  RSS_ACTIVE_COLOR,
  RSS_BG_COLOR,
...
export function RSSFeed() {
  ...
  const [rssActiveColor] = useStorage(
    RSS_ACTIVE_COLOR,
    RSS_ACTIVE_COLOR__DEFAULT,
  );
  const [rssActive] = useStorage(ACTIVE_RSS_FEED, {} as IRSSFeed);
  const [rssFeeds] = useStorage(RSS_FEEDS, [] as IRSSFeed[]);

  const [selectedFeed, setSelectedFeed] = useState<IRSSFeed>(rssActive);
    ...
    <Fragment>
      {strToBool(rssShow) && (
        <div className="w-3/4 flex flex-col items-start justify-center">
            ...
            {rssFeeds.map((feed) => (
              <button
                ...
                style={{
                  ...
                  color:
                    selectedFeed.link === feed.link
                      ? getTextColor(rssActiveColor)
                      : getTextColor(rssBackgroundColor),
                }}
          ...
          <div
            id="rss"
            className="space-y-2 h-[70vh] overflow-y-auto pr-4"
            style={
              {
          ...
          >
            {feedItems?.map((item) => (
              <a
                ...
                style={
                  {
                    backgroundColor: rssBackgroundColor,
                    "--tw-ring-color": rssActiveColor,
                  } as React.CSSProperties
                }
              >
                <div
                  className={`text-lg font-semibold`}
                  style={{ color: rssTitleColor }}
                >
                  {item.title}
                </div>
                <div
                  className={`text-md italic`}
                  style={{ color: rssDateColor }}
                >
